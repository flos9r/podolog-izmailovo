...
export async function PUT(request: Request) {
  ...
  const settings = await prisma.siteSettings.update({
    ...
    data: {
      ...
      heroImageZoom: data.heroImageZoom != null ? parseFloat(data.heroImageZoom) || 1.0 : 1.0,
      aboutImageFit: data.aboutImageFit,
      aboutImagePosition: data.aboutImagePosition,
      aboutImageZoom: data.aboutImageZoom != null ? parseFloat(data.aboutImageZoom) || 1.0 : 1.0,
